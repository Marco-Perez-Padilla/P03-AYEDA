...
template<> class BigInteger<2> : public BigNumber<2> {
  ...
  * @return ostream
  */
 std::ostream& BigInteger<2>::write(std::ostream& os) const {
   if (module_.empty()) {
     os << '0';
   } else {
     if (sign_ == 0) { // If negative, print "-" before the number
       os << "-";
     }
     for (int i {module_.size() - 1}; i >= 0; --i) {
       bool digit = module_[i];
       os << digit;  
     }
   }
   return os;
 ...
 */
template <unsigned char Base = 10> class BigInteger : public BigNumber<Base> {
 private:
 ...
 * @return ostream
 */
template <unsigned char Base> std::ostream& BigInteger<Base>::write(std::ostream& os) const {
  if (module_.getDigits().empty()) {
    os << '0';
  } else {
    if (sign_ == 0) { // If negative, print "-" before the number
      os << "-";
    }
    for (int i {module_.getDigits().size() - 1}; i >= 0; --i) {
      // Same as with BigUnsigned, we get the representation of the letters when the base is greater than 10
      if (module_.getDigits()[i] >= 'A' && module_.getDigits()[i] <= 'Z') {
        os << module_.getDigits()[i];
      } else {
        int digit = module_.getDigits()[i];
        os << digit;
      }
    }
  }
 ...
 * @return istream
 */
template <unsigned char Base> std::istream& BigInteger<Base>::read(std::istream& is) {
  std::string input;
  is >> input;
  // Initialize a bool to create the BI number
  bool not_negative = true;
  ...
  *this = BigInteger<Base>(BigUnsigned<Base>(reinterpret_cast<const unsigned char*>(input.data())), not_negative);
